...
-- 封禁信息分区
local BanSection = Window:Section({
    Title = "封禁信息（snow）",
    Opened = true,
    BackgroundColor3 = Color3.fromRGB(50, 50, 50),
    Height = UDim.new(0, 200),
})
local BanTab = BanSection:Tab({
    Title = "封禁状态",
    Icon = "rbxassetid://7734068321",
    Desc = "账号封禁信息查询",
...
local banReasonLabel = BanTab:Label("封禁原因：无")
local banCountLabel = BanTab:Label("封禁次数：无")
local isBannedLabel = BanTab:Label("是否封禁：否")
local banTimeLabel = BanTab:Label("封禁时间：无")
local unbanTimeLabel = BanTab:Label("解封时间：无")
